...
def main():
    ...
    print(Fore.BLUE, f" 16 bytes of token in hex format => {secrets.token_bytes(nbytes=16).hex()}")

    # generate a random URL safe string
    print(Fore.BLUE, f" 16 bytes of URL safe token => {secrets.token_urlsafe(nbytes=16)}")
    ...
    """
    print(Fore.GREEN, f" return a CSPRNG random int in the range [0 to upper bound number] -> {secrets.randbelow(17)}")
    # use bin() function to convert the integer to binary. number starting with '0b', indicating binary
    print(Fore.GREEN,
          f" return a CSPRNG random k nos of bits -> {bin(secrets.randbits(3))}, random number = {secrets.randbits(3)}")

    # generate a random number: this is not cryptographically secure
    print(Fore.GREEN, f" return a PRNG random number between a range: 5 to 10 -> {random.randint(5, 10)}")

    print(Fore.BLUE, f" different use cases of secrets functions")
    # Generate an eight-character alphanumeric password:
    import string  # used to get the entire alphanumeric character set
    ...
    with open('/usr/share/dict/words') as f:  # each line containing a word but followed by a blank line
